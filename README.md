...
## AHMET — AHMETALIUNAL.COM.TR BAĞLAMA

Cloudflare Workers → `yeni-depo1` → Settings → Domains & Routes bölümünden `ahmetaliunal.com.tr` alan adını bu Worker’a ekle. Cloudflare’ın gösterdiği DNS kayıtlarını kullan; A/CNAME değerlerini elle tahmin etme. Alan adı açıldıktan sonra Supabase Auth URL Configuration içindeki Site URL ve Redirect URLs değerlerini güncelle. Canonical, sitemap ve robots adresleri istek alan adına göre üretilir; farklı bir canonical tercih edilirse Admin → Site ayarları içindeki site adresini kullan.
